...
class ProfitLossReport(models.TransientModel):
    ...
    start_date = fields.Date(string="Start date",
                             help="From which date do you want to generate"
                                  "report")
    end_date = fields.Date(string="End date",
                           help="To which date do you want to generate the"
                                "report")
        ...
        if self.start_date and self.end_date:
            if self.start_date > self.end_date:
                raise ValidationError(
                    _("Please check the date that you " "provide"))
    ...
    def action_button_to_print_pdf(self):
        ...
        query += (
            "group by account_account.code_store ->> '1' ,account_account.name ->> 'en_US' ,"
       ...
       """
        ...
        if self.end_date:
            ...
            '{self.end_date}' """
        query_other_income += (
            " group by account_account.code_store ->> '1' "
            ",account_account.name ->> 'en_US' ,account_account.account_type"
        )
        ...
        net_profit = ((total_op_income + total_other_income)
                      - total_cor - net_exp - net_dep)
        ...
        data = {
            ...
            "today": fields.Datetime.now().strftime("%Y-%m-%d %H:%M"),
        }
        return self.env.ref(
            "profit_and_loss_pdf_report.pl_report_pdf").report_action(
            self, data=data)
