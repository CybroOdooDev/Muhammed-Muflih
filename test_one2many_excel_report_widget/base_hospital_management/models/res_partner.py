...
class ResPartner(models.Model):
    ...
    def _compute_invoice_count(self):
        """Compute the total count of invoices for this patient"""
        for partner in self:
    ...
    def action_generate_patient_card(self):
        ...
        if not self.barcode:
            ...
            with open('code.png', 'rb') as f:
                self.sudo().write({
                    'barcode_png': base64.b64encode(f.read()).decode('utf-8')
                })
        if self.gender:
        ...
        data = {
            ...
            'blood': blood_caps + (str(self.rh_type) if self.rh_type else ''),
    ...
    def action_get_patient_data(self, patient_id):
        ...
        if not patient_id:
            return {'name': 'Patient Not Found'}
        patient_str = str(patient_id).strip()
        data = self.sudo().search([
            '|', '|', '|',
            ('patient_seq', '=ilike', patient_str),
            ('barcode', '=', patient_str),
            ('phone', '=', patient_str),
            ('name', '=ilike', patient_str)
        ], limit=1)
        patient_history = []
        if data:
        ...
        values = {
            ...
            'phone': data.phone if data and data.phone else '',
            'dob': str(data.date_of_birth) if data and data.date_of_birth else '',
            'image_1920': data.image_1920 if data else False,
            'status': data.marital_status if data and data.marital_status else '',
            'history': patient_history,
        }
        if data and data.blood_group:
            blood_caps = data.blood_group.capitalize()
            values['blood_group'] = blood_caps + (str(data.rh_type) if data.rh_type else '')
        else:
            values['blood_group'] = ''
        if data and data.gender:
            gender_caps = data.gender.capitalize()
            values['gender'] = gender_caps
            values['gender_raw'] = data.gender
        else:
            values['gender'] = ''
            values['gender_raw'] = ''
        return values
