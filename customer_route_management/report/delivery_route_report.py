...
class RouteReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        from the wizard"""
        route_ids = data.get('route_ids', []) if data else []
        return {
            'docs': self.env['delivery.route'].search([('id', 'in', route_ids)]),
            'pay': data.get('payment', False) if data else False,
            'consolidated': data.get('consolidated', False) if data else False,
        }
