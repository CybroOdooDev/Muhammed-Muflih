...
class StockLot(models.Model):
    ...
    def _pharma_create_disposition_transfer(self, dest_location):
        ...
        # The Quarantine location is commonly a *child* of the main stock
        # location (e.g. WH/Stock/Quarantine under WH/Stock). If the Released
        # Stock Location is that parent (WH/Stock), a `child_of` exclusion would
        # treat the quarantined quant as "already at destination" and skip the
        # release move entirely — so approved incoming stock would never leave
        # Quarantine. Matching the exact destination relocates it correctly.
        ...
        if not picking_type:
            ...
            # the approved lot would stay stuck in Quarantine, so fall back to
            # an archived internal type rather than skip the release move.
