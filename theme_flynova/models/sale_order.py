...
class SaleOrder(models.Model):
    ...
    booking_date = fields.Date(string='Booking Date',help="The check-in or start date of the booking, set from the customer's selection on the website.")
    booking_date_end = fields.Date(string='Booking Date End', help="The check-out or end date of the booking. If not set, the booking is treated as a single-day event.")
    adult_qty = fields.Integer(string='Adults', default=1, help="Number of adult guests (18+) included in this booking.")
    child_qty = fields.Integer(string='Children', default=0, help="Number of child guests (under 18) included in this booking.")
    is_booking_order = fields.Boolean(
        string='Is Booking Order', default=False,help="When enabled, this sale order represents a Flynova tour/hotel booking and will generate an event upon confirmation.")
    booking_product_id = fields.Many2one(
        'product.template', string='Booking Product', help="The tour or hotel product associated with this booking order."  )
