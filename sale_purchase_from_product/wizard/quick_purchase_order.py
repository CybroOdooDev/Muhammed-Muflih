...
class QuickPurchaseOrder(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        wizard_line = []
        for rec in products:
            wizard_line.append(Command.create({
                'product_id': rec.id,
                'product_qty': 1,
                'price_unit': rec.lst_price,
                'tax_ids': [Command.set(rec.taxes_id.ids)],  # tax_id is the field in quick.purchase.line
            }))
        res.update({
    ...
    def action_create(self):
        ...
        order = self.env['purchase.order'].sudo().create({
            ...
            'order_line': [Command.create({
                ...
                'tax_ids': [Command.set(rec.tax_ids.ids)] if rec.tax_ids else [],
