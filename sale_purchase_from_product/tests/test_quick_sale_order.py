...
class TestQuickSaleOrder(TransactionCase):

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.customer = cls.env['res.partner'].create({
            'name': 'Quick Sale Test Customer',
        })
        cls.sale_tax = cls.env['account.tax'].create({
            'name': 'Quick Sale Order Tax 10%',
            'amount': 10.0,
            'amount_type': 'percent',
            'type_tax_use': 'sale',
        })
        cls.product = cls.env['product.product'].create({
            'name': 'Quick Sale Order Product',
            'list_price': 125.0,
            'taxes_id': [Command.set(cls.sale_tax.ids)],
        })
        cls.payment_term = cls.env['account.payment.term'].create({
            'name': 'Quick Sale Payment Term',
        })
    ...
    def _create_wizard(self, **values):
        ...
        return self.env['quick.sale.order'].create(wizard_values)

    def test_sale_order_domain(self):
        """Only draft and sent sale orders are selectable."""
        wizard_model = self.env['quick.sale.order']

        self.assertEqual(
            wizard_model._sale_ids_domain(),
            [('state', 'in', ['draft', 'sent'])],
        )
    ...
    def test_default_get_creates_lines_from_active_products(self):
        ...
        wizard = self.env['quick.sale.order'].with_context(
        ...
        self.assertEqual(wizard.line_ids.product_uom_qty, 1.0)
        self.assertEqual(wizard.line_ids.price_unit, self.product.lst_price)
        self.assertEqual(wizard.line_ids.tax_ids, self.sale_tax)

    def test_action_create_sale_order(self):
        """The wizard creates a sale order with matching line values."""
        order_date = fields.Datetime.now()
        wizard = self._create_wizard(order_date=order_date, payment_term_id=self.payment_term.id)
        ...
        self.assertEqual(order.order_line.tax_ids, self.sale_tax)

    def test_action_update_existing_sale_order(self):
        """Wizard lines are appended to every selected sale order."""
        sale_order = self.env['sale.order'].create({
            'partner_id': self.customer.id,
        })
        wizard = self._create_wizard(
            type='existing_order',
            sale_ids=[Command.set(sale_order.ids)],
        )
        ...
        self.assertEqual(sale_order.order_line.price_unit, 115.0)
        self.assertEqual(sale_order.order_line.tax_ids, self.sale_tax)

    def test_action_create_view(self):
        """Create-and-view returns the sale order form action."""
        wizard = self._create_wizard()
