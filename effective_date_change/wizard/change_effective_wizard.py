    ...
    def update_effective_date(self):
        ...
        for picking in self.env['stock.picking'].browse(self._context.get('active_ids', [])):
            ...
            if 'stock.valuation.layer' in self.env:
                self._cr.execute("UPDATE stock_valuation_layer SET create_date = (%s) WHERE description LIKE (%s)",
            ...
            self._cr.execute("UPDATE account_move_line SET date = (%s) WHERE ref SIMILAR TO %s",
                                [selected_date, str(picking.name + "%")])

            # Mengganti tanggal account.move
            self._cr.execute("UPDATE account_move set date = (%s) WHERE ref SIMILAR TO %s",
                                [selected_date, str(picking.name + "%")])
            ...
            def update_journal_name(selected_prefix, picking_name):
                """Update sequence prefix, sequence number, and entry name for account moves.

                :param selected_prefix: Sequence prefix based on journal code and new date period.
                :param picking_name: Reference name of the active stock picking.
                """
