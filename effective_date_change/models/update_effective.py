    ...
    def button_validate(self):
        ...
        if self.date_of_transfer != False:
            ...
            if 'stock.valuation.layer' in self.env:
                self._cr.execute("UPDATE stock_valuation_layer SET create_date = (%s) WHERE description LIKE (%s)", [selected_date, str(self.name + "%")])
            ...
            self._cr.execute("UPDATE account_move_line SET date = (%s) WHERE ref SIMILAR TO %s", [selected_date, str(self.name + "%")])
            ...
            self._cr.execute("UPDATE account_move set date = (%s) WHERE ref SIMILAR TO %s", [selected_date, str(self.name + "%")])
