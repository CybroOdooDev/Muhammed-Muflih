...
const CELL = {
  full:       { label: '✓',  cls: 'cell-full',       title: 'Worked full day' },
  half:       { label: '½',  cls: 'cell-half',        title: 'Worked half day (half-day leave)' },
  leave_full: { label: 'L',  cls: 'cell-leave-full',  title: 'Full day leave' },
  leave_half: { label: '½L', cls: 'cell-leave-half',  title: 'Half day leave' },
}

const XLSX_LABEL = {
  full:       'Full Day Report',
  half:       'Half Day Report',
...
function exportXlsx(data) {
  ...
  const titleRow  = [`Work Report - ${label}`, ...Array(totalCols - 1).fill('')]
  ...
  ws['!cols']   = [{ wch: 24 }, ...data.day_meta.map(() => ({ wch: 16 }))]

  const wb = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(wb, ws, label)
  XLSX.writeFile(wb, `Work_Report_${data.month}.xlsx`)
}
              ...
              {data.day_meta.map((d) => {
                ...
                return (
                  <td
                    ...
                    title={cell?.title || (d.weekend ? 'Weekend' : 'No report')}
                  >
                    {cell ? (
...
export default function ReportDashboard() {
  ...
  // Phase 1 — current month fast; Phase 2 — rest of this year in background
  useEffect(() => {
    const curY   = today.getFullYear()
    const curM   = today.getMonth() + 1
    const curKey = `${curY}-${String(curM).padStart(2, '0')}`
    api.get(`/gmail/sync/?month=${curKey}`)
      .then((r) => {
        setMonthMap({ [r.data.month]: r.data })
        setFetched(true)
        // Phase 2: load remaining months of this year silently
        setLoadingAll(true)
        api.get('/gmail/sync/')
          .then((r2) => {
            const map = {}
            for (const m of (r2.data.months || [])) map[m.month] = m
      ...
      .catch((e) => {
        ...
        setError(msg === 'Gmail not connected.'
  ...
  // monthMap intentionally omitted from deps (only fire on year/month change).
  useEffect(() => {
    if (!fetched) return
  ...
  return (
    <div>
      ...
      <div className="d-flex justify-content-between align-items-start mb-4 gap-3 flex-wrap">
        <div>
          <h4 className="fw-bold mb-1">Report Dashboard</h4>
          <p className="text-muted mb-0">Work reports and leave requests fetched from Gmail.</p>
        </div>

        {/* Month + Year picker — top right */}
        <div className="d-flex flex-column align-items-end gap-1">
          <label className="small text-muted mb-0 d-flex align-items-center gap-1" style={{ letterSpacing: '0.03em' }}>
            Select Month
            {loadingAll && (
              <span className="spinner-border spinner-border-sm text-secondary ms-1"
                style={{ width: 10, height: 10, borderWidth: 2 }}
                title="Loading older months…" />
            )}
          </label>
      ...
      <div className="d-flex gap-3 mb-4 flex-wrap">
        {[
          { cls: 'cell-full',       label: '✓',  text: 'Worked full day' },
          { cls: 'cell-half',       label: '½',  text: 'Worked half day' },
          { cls: 'cell-leave-full', label: 'L',  text: 'Full day leave' },
          { cls: 'cell-leave-half', label: '½L', text: 'Half day leave' },
          { cls: 'cell-missing',    label: '–',  text: 'Not reported' },
        ].map((l) => (
      ...
      {/* ── Loading (initial fetch or month switch) ───────────────────── */}
      {showSpinner && (
        <div
          className="d-flex flex-column align-items-center justify-content-center gap-3"
          style={{ minHeight: 320 }}
        >
          <div className="spinner-border text-primary" style={{ width: 48, height: 48, borderWidth: 4 }} />
          <span className="text-muted">
            {loading ? 'Fetching work reports from Gmail…' : 'Loading…'}
          </span>
        </div>
      )}
      ...
      {!showSpinner && !error && Object.keys(monthMap).length === 0 && (
        <div className="text-center py-5">
          ...
          <p className="text-muted mb-0">No work reports found in Gmail.</p>
          <p className="text-muted small">Make sure work report emails are in your inbox.</p>
        </div>
      )}
      ...
      {!showSpinner && !error && Object.keys(monthMap).length > 0 && !current && (
        <div className="text-center py-5">
          ...
          <p className="fw-semibold text-muted mb-1">No work reports found for {monthLabel(selKey)}.</p>
          <p className="text-muted small mb-0">No work report emails received in this month.</p>
        </div>
      )}
