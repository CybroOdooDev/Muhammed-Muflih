...
class ReturnCustomerPortal(CustomerPortal):
        ...
        if is_internal:
            if show_so_q:
                number_order = request.env[
                    'ir.config_parameter'
                ].sudo().get_param('portal_dashboard.sale_count', 5)
            ...
            if show_po_rfq:
                number_po = config_parameters.get_param(
                    'portal_dashboard.purchase_count', 5)
    ...
    def account(self, **post):
        ...
        google_api = params.get_param('base_geolocalize.google_map_api_key')
        geo_provider = params.get_param('base_geolocalize.geo_provider') or 'openstreetmap'
        
        # Ensure 'error' is in qcontext to avoid KeyError in address_form_fields
        if hasattr(res, 'qcontext') and 'error' not in res.qcontext:
            res.qcontext['error'] = {}
    ...
    @http.route(['/geo/change/<coordinates>'], type='jsonrpc', auth="none",
    ...
    def geo_changer(self, coordinates):
        ...
        try:
            ...
            geolocator = Nominatim(user_agent="Odoo_AllInOneWebsiteKit_Cybrosys", timeout=10)
        ...
        if location:
            ...
            country = pytz.country_names.get(country_code, "Undefined")
    ...
    @http.route(['/geo/location/<address>'], type='jsonrpc', auth="none",
                website=False, csrf=False)
    def geo_location(self, address):
        ...
        """
        locator = Nominatim(user_agent="Odoo_AllInOneWebsiteKit_Cybrosys", timeout=10)
        try:
    ...
    @http.route('/portal/dashboard/data', type='jsonrpc', auth="user", website=True)
    def portal_dashboard_data(self):
        """Returns data for the portal dashboard graphs"""
        user = request.env.user

        # Portal users see their own documents (partner_id based usually, but user_id for internal users)
        # Using the same logic as standard portal controllers for consistency
        check_user = lambda m: [('user_id', '=', user.id)] if user.has_group('base.group_user') else [('message_partner_ids', 'child_of', [user.partner_id.id])]
        
        # Sales
        so_count = request.env['sale.order'].search_count(check_user('sale.order') + [('state', 'in', ['sale', 'done'])])
        quotation_count = request.env['sale.order'].search_count(check_user('sale.order') + [('state', 'in', ['draft', 'sent'])])
