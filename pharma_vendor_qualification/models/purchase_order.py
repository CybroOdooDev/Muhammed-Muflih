...
class PurchaseOrder(models.Model):
    ...
    def _compute_approved_vendor_ids(self):
        """Assign the vendors with an approved status in the AVL."""
        for order in self:
            avls = self.env['pharma.avl'].search([('status', '=', 'approved')])
            order.approved_vendor_ids = avls.mapped('vendor_id')
    ...
    # ── AVL Check ─────────────────────────────────────────────────────────────
    def _check_avl(self):
        """Verify each pharma line's vendor is approved on the AVL."""
