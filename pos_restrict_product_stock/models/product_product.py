...
class ProductProduct(models.Model):
    ...
    def _load_pos_data_read(self, records, config):
        ...
        _logger.info(
            ...
            getattr(config, 'name', 'N/A'),
            picking_type.display_name if picking_type else 'N/A', picking_type.id if picking_type else None,
            location.display_name if location else 'N/A', location.id if location else None,
            location.complete_name if location else 'N/A',
            location.location_id.complete_name if location and location.location_id else 'N/A',
        )
        if not location:
            _logger.warning(
